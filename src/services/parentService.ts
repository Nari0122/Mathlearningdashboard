...
export const parentService = {
    ...
    /** 해당 학생이 studentIds에 포함된 학부모 목록 (연동된 학부모. parents 컬렉션만 사용) */
    async getParentsLinkedToStudent(studentDocId: string): Promise<{ uid: string; name: string }[]> {
        ...
        try {
            ...
            return snapshot.docs.map((doc) => ({
                uid: doc.id,
                name: (doc.data().name as string) || "학부모",
            }));
        } catch (error) {
